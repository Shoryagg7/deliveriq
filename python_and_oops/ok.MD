...
- In `alembic/env.py`: import `Base` **and both models** (so autogenerate sees them), set `target_metadata = Base.metadata`.
```python
...
```
Remove any `Base.metadata.create_all()` from `main.py` — Alembic owns the schema now. Confirm `alembic_version` table exists in Postgres.
...
### 📈 The distributed scale-up (Redis sorted set) — know this, build later if time
